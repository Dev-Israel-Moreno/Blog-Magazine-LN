...
class Frontend {
	...
	public function get_display_map( $style ) {
		...
		switch ( $style ) {
			case 'dpt-list1':
				$d = array( 'thumbnail-medium', array( 'title', 'meta', 'excerpt', 'category' ) );
				break;
			case 'dpt-list2':
				$d = array( 'thumbnail-medium', array( 'category', 'title', 'meta' ) );
				break;
			case 'dpt-grid1':
				$d = array( 'thumbnail-medium', array( 'category','title', 'meta', 'excerpt' ) );
				break;
			case 'dpt-grid2':
	...
	public function render_entry( $items, $args ) {
		foreach ( $items as $item ) {
			if ( is_array( $item ) ) {
				...
				switch ( $item ) {
					case 'title':
                       // echo the_field('nombre_de_la_columna');
						
						//echo get_the_author_meta( 'user_columna' );
    ...
    public function column() {
       printf(
			'<div class="dpt-column2">',
		    the_field('nombre_de_la_columna'),
            '</div>'
		);
	...
	public function excerpt( $args ) {
		...
		$exrpt_text  = esc_html__( 'Leer m&aacute;s ', 'display-post-types' );
